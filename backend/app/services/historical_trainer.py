...
class HistoricalTrainer:
    ...
    async def train(
        ...
        from_date = from_date or (today - timedelta(days=60)).isoformat()
        payload = await self.client.historical_candles(instrument_key, "minutes", interval, to_date, from_date)
        candles = self._parse_candles(payload)
        samples = self._generate_scalp_samples(symbol, candles, target)
